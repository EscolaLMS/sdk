...
const Schedules: React.FC = () => {
  const { fetchSchedules, schedules } = useContext(EscolaLMSContext);

  return (
    <div>
      <Login />
      <div>{schedules.loading ? "Schedules Loading" : "Schedules Loaded"}</div>
      ...
      <div data-testid="schedules">
        {JSON.stringify(schedules.list, null, 1)}
      </div>
    </div>
  );
};
